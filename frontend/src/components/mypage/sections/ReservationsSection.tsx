...
export function ReservationsSection({
  ...
  const openPrintVoucher = (item: MyReservationItem) => {
    if (!item.ticketPrintable) {
      cinemaAlert("교환권 출력 가능 시간이 지나 출력할 수 없습니다.", "NOTICE");
      return;
    }
    ...
    if (!printWindow) {
      cinemaAlert("팝업이 차단되어 교환권을 열 수 없습니다. 팝업을 허용해 주세요.", "NOTICE");
      return;
    }
        ...
        <head>
          ...
          <style>
            * { box-sizing: border-box; }
            ...
            .qr-box { padding: 16px; border: 1px solid rgba(26, 26, 26, 0.1); border-radius: 12px; background: #fff; display: flex; flex-direction: column; align-items: center; width: 140px; }
        ...
        <div className="mt-8 rounded-sm border border-black/5 bg-[#FDFDFD] p-5 shadow-xl">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            ...
            <label className="flex items-center gap-2 text-[#1A1A1A]">
              <input type="radio" checked={historyType === "current"} onChange={() => setHistoryType("current")} />
              예매내역
            </label>
            <label className="flex items-center gap-2 text-[#1A1A1A]">
              <input type="radio" checked={historyType === "past"} onChange={() => setHistoryType("past")} />
              지난내역
            </label>
            <select
                ...
                return (
                  <div key={item.reservationId} className="rounded-sm border border-black/5 bg-[#FDFDFD] p-6 shadow-xl">
                    ...
                    <div className="mt-4 flex justify-end gap-2">
                      {canPay && (
                        <button className="rounded-sm bg-[#1A1A1A] px-5 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#B91C1C]" onClick={() => onClickPay(item.reservationId)}>
                          결제하러 가기
                        </button>
                      )}
                      
                      {!canPay && item.ticketPrintable && (
                        <button className="rounded-sm border border-[#B91C1C] bg-white px-5 py-2 text-sm font-semibold text-[#B91C1C] transition-colors hover:bg-[#B91C1C] hover:text-white" onClick={() => openPrintVoucher(item)}>
                          교환권출력
                        </button>
                      )}
          ...
          <div className="mt-4 overflow-hidden rounded-sm border border-black/5 bg-white shadow-xl">
            ...
            {cancelledReservations.length === 0 ? (
              ...
              cancelledReservations.map((item) => {
                ...
                return (
                  <div key={item.reservationId} className="grid grid-cols-[1.2fr_1fr_1fr_1.2fr_1fr_0.9fr] items-center border-t border-black/5 px-4 py-4 text-center text-sm text-[#1A1A1A]">
                    ...
                    <span className="flex justify-center">
                      <span className="rounded-full bg-black/5 px-3 py-1 text-xs font-medium text-black/55">{mapCancelReasonLabel(item.cancelReason)}</span>
                    </span>
                  </div>
                );
              })
                      ...
                      <div className="flex flex-col items-end gap-2 text-right">
                        ...
                        <div className="flex gap-2">
                          ...
                          {!canPay && item.cancellable ? (
                            ...
                            !canPay && <span className="inline-block rounded-sm bg-black/5 px-4 py-2 text-sm text-black/35">환불 불가</span>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                {purchaseRows.length === 0 ? (
                  ...
                  purchaseRows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        <span className="px-2 break-all">{row.reservationNumber != null ? row.reservationNumber : "-"}</span>
                        <span className="leading-relaxed">
                        ...
                        <span className="flex justify-center">
                          <span className={`rounded-full px-3 py-1 text-xs font-medium ${row.isCancelled ? "bg-[#B91C1C]/10 text-[#B91C1C]" : "bg-black/5 text-black/55"}`}>
                            {row.statusLabel}
                          </span>
                        </span>
                      </div>
