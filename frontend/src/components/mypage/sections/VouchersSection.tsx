...
export function VouchersSection({
  ...
  return (
    <section>
      <h1 className="text-5xl font-semibold tracking-tight text-[#1A1A1A]">영화관람권</h1>

      <div className="mt-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="mt-2 text-sm text-black/55">· 보유하신 영화관람권/예매권 내역입니다.</p>
      ...
      <div className="mt-7 flex items-center justify-between">
        <p className="text-lg font-semibold text-[#1A1A1A]">
          <>총 <span className="text-[#B91C1C]">{voucherItems.length}</span>매</>
        </p>
        <select
          className="rounded-sm border border-black/10 bg-white px-4 py-2 text-sm text-[#1A1A1A]"
          value={voucherStatus}
      ...
      <div className="mt-3 overflow-hidden rounded-sm border border-black/5 bg-white shadow-xl">
        <div className="grid grid-cols-3 border-y border-black/5 bg-[#FDFDFD] px-4 py-4 text-center text-[11px] font-bold uppercase tracking-[0.18em] text-black/35">
          <span>관람권명</span>
          <span>유효기간</span>
          <span>사용상태</span>
        </div>
        {voucherLoading ? (
          ...
          <div className="py-8 text-center text-black/45">조회된 관람권 내역이 없습니다.</div>
        ) : (
          voucherItems.map((item) => (
            <div key={item.voucherId} className="grid grid-cols-3 border-t border-black/5 px-4 py-4 text-center text-sm text-[#1A1A1A]">
              <span>{item.name}</span>
      ...
      <div className="mt-8 overflow-hidden rounded-sm border border-black/5 bg-[#FDFDFD] shadow-xl">
        <div className="flex items-center justify-between px-4 py-4 text-[11px] font-bold uppercase tracking-[0.18em] text-black/35">
          <span>이용안내</span>
