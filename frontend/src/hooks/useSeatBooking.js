...
export const useSeatBooking = (screeningId) => {
    //1)데이터보관 및 관리
    const [seats, setSeats] = useState([]); // 가공된 좌석 데이터
    const [screeningInfo, setScreeningInfo] = useState(null); // 영화 정보
    const [selectedSeats, setSelectedSeats] = useState([]); // 선택된 좌석들
    ...
    const getPartnerNumber = useCallback((currentSeat) => {
        ...
        if (indexInGroup === -1 || availableSameTypeSeats.length < 2)
            return null;
    ...
    const toggleSeat = (seat) => {
        ...
        if (isAlreadySelected) {
            setSelectedSeats(prev => {
                ...
                if (pair)
                    return prev.filter(s => s.id !== seat.id && s.id !== pair.id);
    ...
    // 실시간 웹소켓 연결
    useEffect(() => {
        seatService.connect(screeningId, (updatedSeatDto) => {
