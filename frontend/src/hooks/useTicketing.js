...
export const useTicketing = (dateList) => {
    ...
    const [selectedTheatersInfo, setSelectedTheatersInfo] = useState([]);
    ...
    // 날짜 관련 핸들러 분리
    const onDateButtonClick = (date) => setSelectedDate(date);
    ...
    const toggleTheater = (id, name) => {
        if (selectedTheaters.includes(id)) {
            ...
            alert("Up to 4 theaters can be selected.");
        }
    };
    const toggleMovie = (id) => {
        if (selectedMovies.includes(id)) {
            ...
            alert("Up to 3 movies can be selected.");
        }
    };
