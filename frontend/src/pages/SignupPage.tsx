...
const SignupPage: React.FC = () => {
    ...
    useEffect(() => {
        if (socialData && socialData.provider) {
            ...
            cinemaAlert(`${providerName} 계정 정보가 입력되었습니다. 나머지 정보를 입력해주세요!`, "SOCIAL SYNCED");
        }
    }, [socialData]);
    ...
    const handleUsernameCheck = async () => {
        if (!memberData.username) {
            cinemaAlert("아이디를 입력해주세요.", "NOTICE");
            return;
        }
        try {
            ...
            if (response.data.available) {
                cinemaAlert("사용 가능한 아이디입니다.", "AVAILABLE");
                setIsUsernameChecked(true);
            } else {
                cinemaAlert("이미 사용 중인 아이디입니다.", "DUPLICATE ID");
                setIsUsernameChecked(false);
            }
        } catch (error) {
            cinemaAlert("중복 체크 중 오류가 발생했습니다.", "ERROR");
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (activeTab === 'MEMBER') {
            ...
            try {
                ...
                if (error.response && error.response.status === 400) {
                    ...
                    if (errorMessage.includes("이메일")) {
                        cinemaAlert("이미 해당 이메일로 가입된 이력이 있습니다.\n로그인 페이지로 이동합니다.", "ACCOUNT EXISTS", () => navigate('/login'));
                    } else {
                        cinemaAlert(errorMessage, "SIGNUP ERROR");
                    }
                } else {
                    cinemaAlert("회원가입 중 오류가 발생했습니다.", "SERVER ERROR");
                }
            }
        } else {
            ...
            try {
                ...
                cinemaAlert("비회원 정보 등록이 완료되었습니다.", "GUEST REGISTERED", () => navigate('/login'));
            } catch (error) {
                cinemaAlert("비회원 등록 중 오류가 발생했습니다.", "ERROR");
            }
        }
    };
